...
// ===============================
// 📌 ALTERAR TIPO DE COLUNA (PERMANENTE)
// ===============================
...
function atualizarTotais(baseIdx, alvoIdx, concIdx) {
  ...
  for (let i = 2; i < linhas.length; i++) {
    ...
    const baseVal = parseFloat(tds[baseIdx]?.textContent.replace(/\./g,'').replace(',', '.'));
    const alvoVal = parseFloat(tds[alvoIdx]?.textContent.replace(/\./g,'').replace(',', '.'));
