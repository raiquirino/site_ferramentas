  ...
  arquivos.forEach(file => {
    ...
    reader.onload = e => {
      ...
      let htmlImpostos = Object.entries(impostosNota)
        .map(([k,v])=>`<tr><td>${k}</td><td>R$ ${fmt(v)}</td></tr>`).join('');
...
function renderTotaisGerais(){
  // Ordena os impostos pelo nome (chave) em ordem alfabética
  let impostosHTML = Object.entries(total)
    .filter(([_, v]) => v > 0)
    .sort(([aKey], [bKey]) => aKey.localeCompare(bKey)) // <-- ordenação alfabética
